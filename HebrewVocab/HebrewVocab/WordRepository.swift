...
class WordRepository {
    ...
    class func getNextSetOfWords() -> Array<HWord> {
        ...
        let nextSet = defaults.integerForKey("currentSet") + 1
        if nextSet >= wordSet.count {
            //no more sets of words - go back to 0
            defaults.setInteger(0, forKey: "currentSet")
            return wordSet[0]
        } else {
            defaults.setInteger(nextSet, forKey: "currentSet")
            return wordSet[nextSet]
        }
    }
    ...
    class func getCurrentSetOfWords() -> Array<HWord> {
        ...
        loadWords()
        let defaults = NSUserDefaults.standardUserDefaults()
        if defaults.integerForKey("currentSet") > 0 {
